package org.mcphoton.packetlib;
...
import org.mcphoton.packetlib.event.server.ServerBoundEvent;
import org.mcphoton.packetlib.event.server.ServerClosedEvent;
import org.mcphoton.packetlib.event.server.ServerClosingEvent;
import org.mcphoton.packetlib.event.server.ServerEvent;
import org.mcphoton.packetlib.event.server.ServerListener;
import org.mcphoton.packetlib.event.server.SessionAddedEvent;
import org.mcphoton.packetlib.event.server.SessionRemovedEvent;
import org.mcphoton.packetlib.packet.PacketHandlersManager;
...
public class NetworkServer {
	...
	private final PacketHandlersManager handlersManager = new PacketHandlersManager();

	public NetworkServer(String host, int port, Class<? extends PacketProtocol> protocol, SessionFactory factory) {
		this.host = host;
		this.port = port;
	...
	/**
	 * @return the PacketHandlersManager.
	 */
	public PacketHandlersManager getHandlersManager() {
	 ...
	 * @throws IllegalStateException If the protocol does not have a no-params constructor or cannot
	 *             be instantiated.
	 */
	...
	/**
	 * Gets the value of the given flag as an instance of the given type. If this session belongs to
	 * a server, the server's flags will be checked for the flag as well.
	 *
	...
	public <T> T getGlobalFlag(String key) {
		...
		try {
			...
			throw new IllegalStateException("Tried to get flag \"" + key
					+ "\" as the wrong type. Actual type: " + value.getClass().getName());
		}
	}

	/**
	 * Sets the value of a flag. The flag will be used in sessions if a session does not contain a
	 * value for the flag.
	 *
	 * @param key Key of the flag.
	...
	public void close(boolean wait) {
		...
		this.listener.close(wait, new Runnable() {
			@Override
			public void run() {
				callEvent(new ServerClosedEvent(NetworkServer.this));
			}
		});
	}
}
